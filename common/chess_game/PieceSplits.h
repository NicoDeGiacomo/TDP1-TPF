...
class PieceSplits {
 public:
  explicit PieceSplits(Piece* piece);
  ...
  std::list<Position> getSplits(const Piece *piece) const;
 ...
 private:
  ...
  void getSplits_(const std::shared_ptr<SplitNode_> &node, const Piece *piece, std::list<Position>& positions) const;
};
